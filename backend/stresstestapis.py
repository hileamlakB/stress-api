...
@router.get("/{test_id}/results", response_model=StressTestResultsResponse)
async def get_test_results(
    test_id: str, 
    db: Session = Depends(get_db), 
    _: None = Depends(verify_email_confirmed)
):
    """Get the results of a stress test"""
    try:
        # Log the results request
        logger.info(f"[RESULTS] Results request for test ID: {test_id}")
        
        # Get task status from task queue
        task_info = get_task_status(test_id)
        if not task_info:
            logger.warning(f"[RESULTS] Test with ID {test_id} not found")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Test with ID {test_id} not found")

        # Check if test has completed or has results
        if task_info.get("status") not in ["completed", "failed"] and "results" not in task_info:
            logger.warning(f"[RESULTS] Test with ID {test_id} is still running or has no results yet")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Test with ID {test_id} is still running or has no results"
            )
            
        # Get test results data
        results_data = task_info.get("results", {})
        
        # Set default data_strategy if missing
        if "data_strategy" not in results_data:
            results_data["data_strategy"] = "random_each_time"
            
        # Process results
        if results_data:
            logger.info(f"[RESULTS] Retrieved results for test {test_id}")
            
            # Log summary stats if available
            if "summary" in results_data:
                summary = results_data.get("summary", {})
                logger.info(f"[RESULTS] Summary for test {test_id}: "
                           f"total={summary.get('total_requests', 0)}, "
                           f"success={summary.get('successful_requests', 0)}, "
                           f"failed={summary.get('failed_requests', 0)}, "
                           f"avg_time={summary.get('avg_response_time', 0):.2f}ms")

        # Get test status
        status_val = task_info.get("status")
        if status_val not in [s.value for s in TestStatus]:
            status_val = TestStatus.PENDING
        else:
            status_val = TestStatus(status_val)
        ...
        return StressTestResultsResponse(
            ...
            data_strategy=results_data.get("data_strategy", "random_each_time")
        )
    except HTTPException:
        raise
    except Exception as e:
